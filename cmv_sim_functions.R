...
# given a CFA lavaan model, add a CMV marker variabled that loads on all measures returns 
...
add_cmv_marker <- function(model, marker_name = "CMV", marker_measures = 7, marker_loading = .9){
  ...
  new_model <- rbind(measures,                #put the free parameters back together
                     cmv_measures)
  new_model$free <- 1:nrow(new_model) #number them
  ...
  new_model$id <- 1:nrow(new_model) #add the id back in
  new_model <-
...
test_fix_loadings <- function() {
  get_test_model() |> lavaanify() |> fix_loadings("X", 0.7) |> 
    fix_loadings("Y", 0.8) |> fix_loadings("Z", 0.9) |> print()
}
