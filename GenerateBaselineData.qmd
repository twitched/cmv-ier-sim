...
semPlotModel_lavaanModel(cfa_sim_model) |> 
  semPaths(layout="tree2", rotation=2, nCharNodes=5, sizeMan2 = 2.5, mar = c(1,4,1,3), label.norm = "OOOOO", intAtSide = TRUE, levels = c(1,2,7,8), curvature = 2, residuals = FALSE)
```
...
-   $β_i$ is the intercept for item $i$ or the mean

-   $η_{jk}$ is the $k$th common factor for person $j$.

-   $λ_{ik}$ is the factor loading of item $i$ on factor $k$.

-   $ϵ_{ij}$ is the random error term for person $j$ on item $i$.

-   Ψ is the variance-covariance matrix of the common factors $η_j$.

-   Θ is the variance-covariance matrix of the residuals (or unique factors) $ϵ_j$.
